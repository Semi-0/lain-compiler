(ns propagators.compiler.operators.reducer
  "Compiler-2 closures adapted to reducer-subnet merge networks."
  (:require [propagators.compiler.model.closure-value :as closure-value]
    ...
    (when arg-ids
      (let [n0 (-> net/empty-net
            ...
            [_prop-id n1] (((requiring-resolve
                             'propagators.compiler.lowering.application/p:apply-closure)
                            closure-cell-id
                            args-id
                            arg-ids
                            out-id)
