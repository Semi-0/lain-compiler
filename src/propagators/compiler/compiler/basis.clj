(ns propagators.compiler.compiler.basis
  ...
  (:require [clojure.core :as core]
            ...
            [propagators.infra.datastructures.behavior.arithmetic :as behavior-arithmetic])
   ...
   {:name 'execute-sub-env
    ...
    (fn [compile* network _context-id arg-ids out-id]
      (let [[expr-id parent-env-id & watch-ids] (vec arg-ids)
        ...
        ((requiring-resolve
          'propagators.compiler.lowering.application/execute-sub-env-messages-with)
         compile*
         parent-env-id
         expr-id
         child-env-id
    ...
    :activate (fn [network _context-id arg-ids out-id]
                (let [[expr-id parent-env-id & _watch-ids] (vec arg-ids)
                      child-env-id (stable-node-id :compiler-2 :execute-sub-env out-id)]
                  ((requiring-resolve
                    'propagators.compiler.lowering.application/execute-sub-env-messages)
                   parent-env-id
                   expr-id
                   child-env-id
                   out-id
...
(defn- sync-update
  ...
  (let [content (net/network-cell-content network id)
    ...
    (cond
      ...
      ((requiring-resolve 'propagators.infra.datastructures.behavior/behavior-value?)
       content)
      content
      ...
      [(message out-id
                (case pred
                  ...
                  :closure ((requiring-resolve
                             'propagators.compiler.model.closure-value/closure-info?)
                            v)
...
(defn- bind-default-tms-operators
  [compiler-env]
  ((requiring-resolve
    'propagators.compiler.operators.tms/bind-distributed-tms-operators)
   compiler-env))
...
(defn behavior-tms-env []
  ((requiring-resolve
    'propagators.compiler.operators.behavior/behavior-tms-env)))

(defn legacy-central-tms-env []
  ((requiring-resolve
    'propagators.compiler.legacy/legacy-central-tms-env)))
