...
Its assembly is in `propagators.compiler.cps-core`; handlers are ordinary
...
- `propagators.compiler.main`: public compatibility facade.
- `propagators.compiler.cps-core`: production compiler assembly and compile
  entrypoints.
- `propagators.compiler.compiler.predicates`: AST predicates.
- `propagators.compiler.compiler.handlers`: CPS traversal handlers.
- `propagators.compiler.compiler.declarations`: topology declarations and
  lowerings.
- `propagators.compiler.compiler.basis`: stable IDs, default operators, and
  operator construction helpers.
- `propagators.compiler.language`: parser and slot-backed AST.
- `propagators.compiler.model`: environments, closure values, application
  values, context, and operator values.
- `propagators.runtime`: public reusable session facade.
- `propagators.runtime.application`: retained application
  evaluation and sub-environment execution.
- `propagators.runtime.closure-frame`: compiler-2 closure frame
  declaration and `p:apply-closure` integration.
- `propagators.runtime.tui`: block models, rewrites, version history,
  and versioned commits.
- `propagators.runtime.boundary`: runtime effect evaluation.
- `propagators.runtime.inspection`: cells, trace subscriptions, and
  temperature data.
- `propagators.compiler.operators`: call graph, distributed TMS, block
  premises, versioned definitions, and behavior bridges.
- `propagators.infra.propagator`, `propagators.infra.network`,
  `propagators.infra.network-builder`, and `propagators.infra.core`: kernel construction and
  evaluation.
- `propagators.tui.graph.*`: presentation, Charm TUIs, socket servers, XR adapters, and the
...
`propagators.compiler.model.operator-value/propagator-operator`:
...
(operator-value/propagator-operator
 {:name 'my-operator
  ...
  (fn [network input-ids output-ids context-id]
    ;; Return a vector of propagators.infra.message/message values.
...
`propagators.infra.propagator/compound-propagator` installs a host-level propagator
whose closure cell contains `propagators.infra.closure/Closure`. On activation it:
...
`propagators.compiler.operators.call-graph` is an additive reactive
...
| Which propagator tasks ran and what messages did they emit? | `propagators.infra.debug/run-tasks-debug`. |
...
`propagators.infra.debugger/with-debugger` plus `set-sink!`/`report!` is a smaller
...
clojure -M -m propagators.tui.graph.compiler-2-runtime-server json-export 45556 instance.json
clojure -M -m propagators.tui.graph.compiler-2-runtime-server json-import 45556 instance.json
```
...
The explicit suite
`propagators.runtime.session.file-loader-test` currently reports 11
...
reported about 55 seconds total commit time, 8,667 cells, and 6,271 propagators.infra.
A later input after 50 definition candidates took about 52 ms and activated
504 propagators.infra. Dormant candidates still receive shared caller inputs; premise
...
Reusable runtime code is moving under `propagators.runtime`, but the
...
clojure -M:test \
  propagators.compiler-closure-frame-test \
  propagators.compiler-composition-test \
  propagators.compiler-cps-test \
  propagators.compiler-organization-test \
  propagators.compiler-gur-linked-list-test

# Core compiler, behavior compiler, distributed TMS
clojure -M:test \
  propagators.infra.compile-2-test \
  propagators.infra.behavior-compiler-test \
  propagators.infra.tms-test

# Call graph
clojure -M:test propagators.compiler-call-graph-test

# Versioned runtime (names now live under propagators.runtime.*)
clojure -M:test \
  propagators.runtime.tui.block-compiler-test \
  propagators.runtime.tui.versioned-commit-test \
  propagators.runtime.session.instance-replay-test

# Presentation/runtime integration
clojure -M:test \
  propagators.tui.graph.vijual.compiler-2-semantic-repl-test \
  propagators.tui.graph.vijual.compiler-2-runtime-server-test

# Explicitly unregistered known-red gate
clojure -M:test \
  propagators.runtime.session.file-loader-test
