...
```bash
clojure -M -m propagators.tui.graph.compiler-2-runtime-server block-list 45555 agent-1
clojure -M -m propagators.tui.graph.compiler-2-runtime-server block-show 45555 agent-1 0
clojure -M -m propagators.tui.graph.compiler-2-runtime-server block-focus 45555 agent-1 0
```
...
```bash
clojure -M:test \
  propagators.runtime.environment-io-test \
