...
- `propagators/compiler_2/operators/call_propagators.tui.graph.clj`
...
the same propagators.tui.graph. Recursive calls point back to the caller closure ID and appear
...
operators live in `propagators.compiler.operators.tms`, behavior operators live in
`propagators.compiler.operators.behavior`, and the old
`propagators.compiler.tms-behavior` namespace is now only a compatibility
...
`propagators.compiler.tms-behavior/legacy-central-tms-env` or the thin
...
`propagators.compiler.main/compile-source` remains the raw compiler-2 entry
...
`propagators.compiler.cps-core/compiler-dispatch` is the production rule
...
under `propagators.compiler.deprecated`; its `g:compile`, `g:apply`, and
...
Verified in `propagators.compiler-composition-test` and
`propagators.compiler-cps-test`, including public MultiFn identity,
...
`propagators.infra.closure/Closure` runtime function:
...
```bash
clojure -M -m propagators.tui.graph.compiler-2-semantic-repl \
'(let-cell [same next] ((network [x] [same next] (<-> x same) (<-> (+ x 1) next)) 4 same next) next)'

clojure -M -m propagators.tui.graph.compiler-2-runtime-server server 45555

clojure -M -m propagators.tui.graph.compiler-2-runtime-server request 45555 \
'{:op :compile/source :source "(let-cell [same next] ((network [x] [same next] (<-> x same) (<-> (+ x 1) next)) 4 same next) next)"}'

clojure -M -m propagators.tui.graph.compiler-2-runtime-server graph 45555
clojure -M -m propagators.tui.graph.compiler-2-runtime-server trace 45555 next

clojure -M -m propagators.tui.graph.compiler-2-runtime-server request 45555 \
'{:op :semantic/trace/install :label "next" :direction :upstream :interval-ms 5000}'

clojure -M -m propagators.tui.graph.compiler-2-runtime-server udp-request 45555 \
'{:op :agent/send-block :client-id "agent" :text "(+ 1 2)"}'

clojure -M -m propagators.tui.graph.compiler-2-runtime-server udp-request 45555 \
'{:op :agent/blocks :client-id "agent"}'

clojure -M -m propagators.tui.graph.compiler-2-tui 45555 tui-1
```
...
`propagators.infra.gur/p:apply-closure`.
...
| fixed | Runtime `trace` closes over a per-block graph sidecar. | `(trace out g)` can trace block plumbing instead of the runtime env propagators.tui.graph. | Bind one stable runtime semantic graph cell in the compiler env and have `trace` read it. | `block-language-traces-def-net-application-dependence-graph`. |
...
| fixed | TUI trace blocks compile before the complete runtime semantic graph exists. | `(trace a :upstream g)` can stay empty when upstream relations are added later. | Recompile trace source blocks after the full source-block rebuild pass and seed the stable runtime graph cell with the accumulated propagators.tui.graph. | `trace-block-reacts-to-later-upstream-relationships`. |
...
`propagators.infra.application/build-branch-application` already moves in this
