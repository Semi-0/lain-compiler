...
`propagators.tui.graph.compiler-2-versioned-tui` is an additive client of the existing
multi-client runtime. It does not replace `propagators.tui.graph.compiler-2-tui`.
...
of the same application; `propagators.runtime.session.instance-replay-test`
...
- `propagators.runtime.tui.block-compiler-test` covers idempotent lowering,
...
```bash
clojure -M -e \
  '(require (quote propagators.tui.graph.compiler-2-versioned-tui-bench))
   (prn (propagators.tui.graph.compiler-2-versioned-tui-bench/benchmark-definition-edits-existing-application-input 50))'
```
